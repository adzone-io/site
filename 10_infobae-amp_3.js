...
t.version_datetime = '2020-Sep-09 04:44:36';
...
t.get_page_path = function() { if(!("path" in adzone)) {
  adzone.path = "";
}

var d = document.querySelectorAll("amp-ad");
if(d.length>0 && adzone.path=="") {
  var slot = d[0].dataset.slot;
  var a = slot.split("/");
  adzone.path = ""; 
  for(var i = 2; i<a.length-1; i++){ adzone.path += "/" + a[i]; }
}


for(var i = 0; i<d.length;i++) {
  var w = d[i].getAttribute("width");
  var h = d[i].getAttribute("height");
  var s = d[i].dataset.slot.split("/");
  
  if(!d[i].dataset.adzone) {
    d[i].dataset.adzone = 1;
    var div = document.createElement("div");
    div.classList.add("ad-slot");
    var adtype = "box";
    if(w==320&&h==50) { adtype = "wide"; }
    div.dataset.adtype = adtype;
    div.id = s[s.length-1];
    d[i].appendChild(div);
  }
}
...
t.kv = (function() { return { "platform": "amp" } })();
t.size_mapping=[{"device": "mobile", "sizes": [[300, 250]], "adtype": "box", "minwidth": 0, "refresh": 0, "adx_refresh": 0, "interval": 0}, {"device": "mobile", "sizes": [[320, 50]], "adtype": "wide", "minwidth": 0, "refresh": 0, "adx_refresh": 0, "interval": 0}];
...
t.set_page_kv = function(){
  ...
  googletag.pubads().setTargeting("platform", t.device.isMobile?"mobile":"desktop");
