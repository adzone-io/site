/* 1RZZCR3Bm5 - America - configuración general */
...
if(document.location.href.indexOf("epl_hide=1")<0){
  adzone.use_epl = true;
  adzone.epl_hb_slots = 'fullcontent, zocalo,itt,topbanner';

  var epl_ads = document.createElement('script');
  epl_ads.id="hbepl";
  epl_ads.type = 'text/javascript';
  epl_ads.src="https://sakimg.e-planning.net/layers/hbdfp.js";
  ...
  if(document.location.href.indexOf("epl_usedivname=1")>0){
    epl_ads.dataset.usedivname = true; 
  }
  document.head.appendChild(epl_ads);
} else{
  //console.log(document.querySelectorAll(".ad-slot"));
}
...
t.styles.footer_close=`<img style='width:24px;height:24px;margin-top:-10px;margin-right:-10px' src="https://cdn.jsdelivr.net/gh/adzone-io/rm@1.0.0/icon/close2.png">`;
...
t.styles.prestitial_close=`<img style='width:48px;height:48px;margin-top:-34px;margin-right:-28px' src="https://cdn.jsdelivr.net/gh/adzone-io/rm@1.0.0/icon/close2.png">`;
...
s.async = true;
s.src = 'https://cdn.jsdelivr.net/gh/adzone-io/rm/adzone.js'+Math.round((new Date()).getTime()/1000/60/60);;
...
if(document.getElementById("www.primiciasya.com/Home/Home_FullContent")) {
  ...
  googletag.cmd.push(function() {
    ...
    Object.keys(prepare).forEach(function(c, index) {
      ...
      if(t.use_epl && t.is_epl_slot(prepare[c])) {      
        if(window.hbepl) {
          console.log("** googletag.display("+prepare[c]+"); ");
          googletag.display(prepare[c]); 
          console.log("** hbepl.callSingleSlot( slot: "+prepare[c]+"); ");
          hbepl.callSingleSlot(t.cs[prepare[c]].slot);
        } else {
          console.log("window.hbepl does not exist", prepare[c]);
          googletag.display(prepare[c]);
          console.log("**** Refresh: ", prepare[c]);
          googletag.pubads().refresh([t.cs[prepare[c]].slot], {changeCorrelator: false});
        }
      } else {

        console.log("**** DFP direct. Display: ", prepare[c]);
        googletag.display(prepare[c]);
        //debugger;
        //t.refresh_slots.push(prepare[c])
        if(t.disable_initial_load) {
          console.log("**** DFP direct. Refresh: ", prepare[c]);
          googletag.pubads().refresh([t.cs[prepare[c]].slot], {changeCorrelator: false});
        }
...
t.is_epl_slot = function(d){
  ...
  if(is_epl) {
    console.log("YES, it's epl", d);
    return true;
  } else {  
    console.log("NOT epl", d);
    return false;
  }
}
