...
if(adzone.use_epl && document.location.href.indexOf("epl_hide=1")<0){
  adzone.use_epl = true;
  adzone.epl_hb_slots = '1X1,flotante,fullcontent,zocalo,itt,topbanner,homeevent,Home_320x50_Mob,Notas_728x90_top';
...
t.version_datetime = '2020-Nov-18 23:43:19';
...
t.styles.prestitial_close=`<img style='float:left;max-width:48px;max-height:48px;width:48px;height:48px;margin-top:-34px;margin-right:-28px;' src="https://cdn.jsdelivr.net/gh/adzone-io/rm@1.0.0/icon/close2.png">`;
  ...
  for(var i = 0; i<ad_slots.length; i++) {
    ...
    if(anchor) {
      if(slot.id && slot.adtype) {
        ...
        slot.class && (d.dataset.adzoneclass =slot.class);
...
function adzoneSpecial(){
    ...
    if(adzone_special && adzone_special.run) {
      ...
      var s = document.createElement("script");
      s.src="https://static.americadigital.com.ar/comercial/fibra/fibra_du.js?v1" ;
      document.head.appendChild(s);
...
isFixed = false;
    ...
    function sideBanner() {
        var $banner = $("#special-banner .banner-wrapper");
        var $bannerContainer = $("#special-banner");
        if ($banner.length > 0) {
            var $body = $("body"),
                $bannerWidth = $banner.outerWidth(),
                $container = $(".widgetContent > .container, .widget-content > .container, [class*='widget'] > .container, .main-content.container, header.main-header .container, .listado-horizontal.container, .notes-x-5 .container"),
                $maxContainerWidth = 0;
                for (var i = 0 ; i < $container.length ; i++ ){
                  if ($($container[i]).outerWidth() > $maxContainerWidth){
                    $maxContainerWidth = $($container[i]).outerWidth();
                  }
                }
                console.log("maxContainerWidth ", $maxContainerWidth);
                $containerWidth = $("body").hasClass('detail') ? $(".main-content.container").outerWidth() : $maxContainerWidth,
                $containerOffset = $($container[0]).offset().left,
                $windowWidth = $(window).width(),
                newMarginForContainer = Math.ceil(
                    ($windowWidth - ($containerWidth + $bannerWidth)) / 2
                ),
                isDetail = $("body").hasClass('detail') ? true : false;
            
            $.each($container, function () {
                $this = $(this);
                $this.animate({
                    marginLeft: newMarginForContainer-10
                }, 100, "swing");
            });

            $bannerContainer.animate({
                marginRight: (-1 * Math.floor(($containerWidth + $bannerWidth) / 2)) + "px"
            }, 100, "swing");
            ...
            function bannerFixed() {
                if (isDetail) {
                    var containerWidthToCompare = $(".main-content.container").outerWidth();
                } else {
                    var containerWidthToCompare = $container.outerWidth();
                }

                if ($containerWidth !== containerWidthToCompare || $bannerWidth !== $banner.outerWidth() || $containerOffset !== $container.offset().left) {
                    $containerWidth = containerWidthToCompare;
                    $bannerWidth = $banner.outerWidth();
                    $containerOffset = $container.offset().left;
                    $bannerContainer.css({
                        // 'margin-right': (-1 * Math.floor(($containerWidth + $bannerWidth) / 2)) + "px"
                    });
                } else {
                    $bannerContainer.css({
                        // 'margin-right': (-1 * Math.floor(($containerWidth + $bannerWidth) / 2)) + "px"
            ...
            /* AcÃ¡ corremos devuelta las funciones por si se hace un resize del windows */
            var resizeTimer;
...
adzone.slotRenderedCallback["www.diariouno.com.ar/Home/HomeEvent_260x650"] = manageResidual;
adzone.slotRenderedCallback["www.diariouno.com.ar/Notas/HomeEvent_260x650"] = manageResidual;
t.forced_slots = [];
...
var section = [];
//var cats = document.querySelectorAll(".category.cat-");

if(document.getElementById("category")!=null){
    var cats = removeAccents(document.getElementById("category").getAttribute("data-value").toLowerCase());
    section.push(cats);
}
  ...
  googletag.cmd.push(function() {
    ...
    Object.keys(prepare).forEach(function(c, index) {
      ...
      if(div.id+"_parent"!=div.parentElement.id) {
         ...
         if(div.dataset.adzoneclass) {
           for(var i=0;i<div.dataset.adzoneclass.split(" ").length;i++) {
             div.classList.add(div.dataset.adzoneclass.split(" ")[i]);
           }
         } 
