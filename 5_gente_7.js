...
t.version_datetime = '2020-Sep-10 03:52:29';
t.supertag_class='ad-slot2';
...
t.styles = t.styles || {}
t.styles.footer_close=`<img style='max-width:20px;max-height:20px;width:20px;height:20px;margin-top:-2px;margin-right:-2px' src="https://cdn.jsdelivr.net/gh/adzone-io/rm@2.1/icon/close_square.png">`;
...
var s = document.createElement("script");
s.async = true;
s.src = 'https://cdn.jsdelivr.net/gh/adzone-io/rm@2.2.0001/adzone.js?'+Math.round((new Date()).getTime()/1000/60/60);;
document.head.appendChild(s);
...
t.get_page_path = function() { // borrar ad slots viejos
var slots = document.querySelectorAll(".ad-slot");
for(var i=0; i<slots.length; i++){
  console.log("remove", slots[i].id);
  slots[i].classList.remove("ad-slot");
  slots[i].id = ""; 
}       
   
var d=document.location.href;
var ds = d.split("/");
if(d.indexOf("/categor")>0) {
  adzone.page = "seccion";
  adzone.section = ds[4];
} else if(d.length<28) {
  adzone.page = "home";
...
for(var i=0;i<c.length;i++) {
  try{
    if(c[i].indexOf("postid-")==0) {
      adzone.page_id = c[i].substring(7);
      break;
    }
  } catch(e) {}   
}

return "/gente_" + adzone.page + "/";
...
var d=document.location.href;
var ds = d.split("/");
var l = d.substring(0,d.length-1).split("/").length;
var seccion = "parati"; 
if(d.indexOf("paparazzi")>0) {
  seccion="paparazzi";
  if(l==3){seccion+="_home"; adzone.section = "home";} else if(l==4) { seccion+="_seccion";  adzone.section = ds[3];} else { seccion+="_nota"; adzone.section = ds[3];}
} else if (d.indexOf("parati")>0) {
  seccion="parati";
  if(l==3){seccion+="_home"; adzone.section = "home"; } else if(l==4) { seccion+="_nota"; adzone.section = "nota"; } else { seccion+="_seccion"; adzone.section = ds[4]; }
} else { //gente
  seccion = "gente"
  if(d.indexOf("/categor")>0) {
    seccion += "_seccion";
...
d.substring(0,d.length-1).split("/").length>2 && (nombre_seccion=d.substring(0,d.length-1).split("/")[3])

// buscar todos los ad slots
var slots = document.querySelectorAll(".ad-slot");
adzone.slot_list = adzone.slot_list || {};
for(var i=0; i<slots.length; i++){
  if(slots[i].id && slots[i].id!="") {
      slots[i].dataset.refresh=0;

    if((slots[i].id in adzone.slot_list && !slots[i].dataset.checked && !slots[i].parentElement.dataset.checked) || slots[i].id.indexOf("position")>0) {
      adzone.l.info("remove", slots[i].id);
      slots[i].classList.remove("ad-slot");
      slots[i].id = "";
    } else {
      slots[i].dataset.checked = 1;
      adzone.slot_list[slots[i].id] = true;    
    } 
  }
}
...
/*
adzone.device.isMobile && document.getElementById("box_1") && document.getElementById("box_1").classList.remove("ad-slot"); 
adzone.device.isMobile && document.getElementById("box_2") && document.getElementById("box_2").classList.remove("ad-slot");




document.getElementById("top_1") && (document.getElementById("top_1").style.width="100%");

window.setTimeout(function() {
document.querySelector("#top_1") && document.querySelector("#top_1").parentElement && document.querySelector("#top_1").parentElement.classList.remove("adzonemobileonly");
},100);

*/

// Fix de wordpress que está haciendo un googletag.pubads().refresh() en algún lugar.
googletag.cmd.push(function() {
  googletag.pubads().refresh = function(){ /* adzone.l.info("REFRESH IGNORED");*/ };
});


var ad_slots = [];
if(adzone.page=="nota") {
  var s = document.querySelectorAll("article p");
  s.length>1 && ad_slots.push({ div: s[0], location: 'before' , id: 'wide_1' , adtype: 'wide' });
  s.length>2 && ad_slots.push({ div: s[2], location: 'after' , id: 'wide_2' , adtype: 'wide' });
  s.length>5 && ad_slots.push({ div: s[5], location: 'after' , id: 'wide_3' , adtype: 'wide' });
  s.length>8 && ad_slots.push({ div: s[8], location: 'after' , id: 'wide_4' , adtype: 'wide' });
  s.length>11 && ad_slots.push({ div: s[11], location: 'after' , id: 'wide_5' , adtype: 'wide' });

  adzone.device.isDesktop && (
       document.querySelector("article aside.sidebar") && ad_slots.push({ div: document.querySelector("article aside.sidebar"), location: 'first' , id: 'box_1' , adtype: 'box' }) 
    && document.querySelector("article .thb-fixed") && ad_slots.push({ div: document.querySelector("article .thb-fixed"), location: 'first' , id: 'box_2' , adtype: 'box', style:"margin-top:40px" }) 
  )
  
} else if(adzone.page=="home") {
    
  adzone.device.isDesktop && ad_slots.push({ div: document.querySelectorAll("section.vc_custom_1566008394714 .vc_column-inner")[1], location: 'first' , id: 'box_2' , adtype: 'box', style:"margin-top:20px" });
  adzone.device.isDesktop && ad_slots.push({ div: document.querySelectorAll("section.vc_custom_1566008394714 .vc_column-inner")[1], location: 'first' , id: 'box_1' , adtype: 'bigbox' });

  ad_slots.push({ div: document.querySelector(".vc_custom_1566356545295"), location: 'before' , id: 'wide_1' , adtype: 'billboard', style:'margin-bottom:-40px' });
  // antes y despues de nota de tapa
  ad_slots.push({ div: document.querySelector(".vc_custom_1566356545295"), location: 'after' , id: 'wide_2' , adtype: 'billboard', style:'margin-top:-40px' });

  // despues de galeria
  ad_slots.push({ div: document.querySelectorAll("section")[5], location: 'after' , id: 'wide_3' , adtype: 'billboard', style:'margin-top:40px; margin-bottom:-40px' });

  // antes y despues de no te lo pierdas
  ad_slots.push({ div: document.querySelectorAll("section")[7], location: 'before' , id: 'wide_4' , adtype: 'billboard', style:'margin-bottom:-40px' });
  ad_slots.push({ div: document.querySelectorAll("section")[7], location: 'after' , id: 'wide_footer' , adtype: 'billboard', style:'margin-top:-40px; margin-bottom:0px' });

} else if(adzone.page=="seccion") {
    
  // sidebar 
  adzone.device.isDesktop && ad_slots.push({ div: document.querySelector(".code-block-3"), location: 'first' , id: 'box_1' , adtype: 'bigbox' });
  
  // debajo de twitter
  adzone.device.isDesktop && ad_slots.push({ div: document.querySelector("#thb_twitter_widget-2"), location: 'after' , id: 'box_2' , adtype: 'box' });
  
  // cada 4 articulos
  ad_slots.push({ div: document.querySelectorAll(".archive-pagination-container article")[3], location: 'after' , id: 'wide_1' , adtype: 'wide', style:"margin-bottom:20px", "class":"ad-slot2 medium-12" });
  ad_slots.push({ div: document.querySelectorAll(".archive-pagination-container article")[7], location: 'after' , id: 'wide_2' , adtype: 'wide', style:"margin-bottom:20px", "class":"ad-slot2 medium-12" });
  ad_slots.push({ div: document.querySelectorAll(".archive-pagination-container article")[11], location: 'after' , id: 'wide_3' , adtype: 'wide', style:"margin-bottom:20px", "class":"ad-slot2 medium-12" });
  ad_slots.push({ div: document.querySelectorAll(".archive-pagination-container article")[15], location: 'after' , id: 'wide_4' , adtype: 'wide', style:"margin-bottom:20px", "class":"ad-slot2 medium-12" });

  // antes del footer  
  ad_slots.push({ div: document.querySelector("footer"), location: 'before' , id: 'wide_footer' , adtype: 'billboard', style:'margin-top:20px' });
    
}


ad_slots.length>0 && t.create_slots(ad_slots); ad_slots = [];

t.forced_slots = [];
t.forced_slots.push({s: `itt`, f: `103`, t: `10`, p: `{"backgroundColor":"white", "autoclose":"8"}`, i:`{}` });
...
t.kv = (function() { var check_tags = function(selector) {
  var t = document.querySelectorAll(selector);
  var tags = [];
  for(var i = 0;i<t.length;i++) {
    if(t.item(i).href && t.item(i).href.toLowerCase().indexOf("no-adx")>0) {
...
var d=document.location.href;
if(d.indexOf("paparazzi")>0) {
  tags = check_tags(".entry-tag");
} else if(d.indexOf("parati")>0) {
  tags = check_tags(".tags a");
} else { //gente
  tags = check_tags(".thb-article-tags a");
}
...
if(document.location.href.toLowerCase().indexOf("diosas")>0) {
  kv.adx = "disabled";
}
return kv; })();
t.size_mapping=[{"device": "desktop", "sizes": [[300, 250], [336, 280], [300, 600]], "adtype": "box", "minwidth": 0, "refresh": 0, "adx_refresh": 0, "interval": 0}, {"device": "desktop", "sizes": [[1, 1], [800, 600], [1000, 540]], "adtype": "itt", "minwidth": 0, "refresh": 0, "adx_refresh": 0, "interval": 0}, {"device": "desktop", "sizes": [[970, 90]], "adtype": "top", "minwidth": 970, "refresh": 0, "adx_refresh": 0, "interval": 0}, {"device": "desktop", "sizes": [[728, 90]], "adtype": "wide", "minwidth": 0, "refresh": 0, "adx_refresh": 0, "interval": 0}, {"device": "desktop", "sizes": [[1, 1], [970, 90], [970, 251], [728, 90]], "adtype": "zocalo", "minwidth": 0, "refresh": 0, "adx_refresh": 0, "interval": 0}, {"device": "desktop", "sizes": [[300, 600]], "adtype": "bigbox", "minwidth": 0, "refresh": 0, "adx_refresh": 0, "interval": 0}, {"device": "desktop", "sizes": [[728, 90], [970, 90]], "adtype": "billboard", "minwidth": 0, "refresh": 0, "adx_refresh": 0, "interval": 0}, {"device": "mobile", "sizes": [[300, 250], [300, 600]], "adtype": "bigbox", "minwidth": 0, "refresh": 0, "adx_refresh": 0, "interval": 0}, {"device": "mobile", "sizes": [[300, 250], [300, 600], [336, 280]], "adtype": "box", "minwidth": 0, "refresh": 0, "adx_refresh": 0, "interval": 0}, {"device": "mobile", "sizes": [[1, 1], [320, 480]], "adtype": "itt", "minwidth": 0, "refresh": 0, "adx_refresh": 0, "interval": 0}, {"device": "mobile", "sizes": [[320, 100], [320, 50], [300, 100], [300, 50]], "adtype": "top", "minwidth": 0, "refresh": 0, "adx_refresh": 0, "interval": 0}, {"device": "mobile", "sizes": [[300, 250], [300, 600]], "adtype": "wide", "minwidth": 0, "refresh": 0, "adx_refresh": 0, "interval": 0}, {"device": "mobile", "sizes": [[1, 1], [320, 50]], "adtype": "zocalo", "minwidth": 0, "refresh": 0, "adx_refresh": 0, "interval": 0}, {"device": "mobile", "sizes": [[300, 250]], "adtype": "billboard", "minwidth": 0, "refresh": 0, "adx_refresh": 0, "interval": 0}];
...
// extra slots
  ...
  (true) &&   ad_slots.push({ div_txt: `document.body`, div: function() { return (document.body || null);} , location: 'first' , id: 'itt' , adtype: 'itt', floating: true, css: '', class: '' });
  (true) &&   ad_slots.push({ div_txt: `document.body`, div: function() { return (document.body || null);} , location: 'first' , id: 'zocalo' , adtype: 'zocalo', floating: true, css: '', class: '' });
  (true) &&   ad_slots.push({ div_txt: `document.querySelector("header.thb-main-header")`, div: function() { return (document.querySelector("header.thb-main-header") || null);} , location: 'after' , id: 'top_1' , adtype: 'top', floating: false, css: '', class: '' });
  (adzone.page == "nota") &&   ad_slots.push({ div_txt: `document.querySelector(".thb-social-footer")`, div: function() { return (document.querySelector(".thb-social-footer") || null);} , location: 'after' , id: 'wide_footer' , adtype: 'wide', floating: false, css: '', class: '' });
...
adzone.setup_interval = window.setInterval(function() {
  if(document.body) {
    window.clearInterval(adzone.setup_interval);
    adzone.setup_supertag();
  }
},10);
