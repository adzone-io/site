...
t.code = 'IsB94kt1D7.200420.2';

t.log = function(t1, t2, t3, t4) {
  if(localStorage.getItem("adzone-debug")) {
    try {
      txt = "Adzone: " + t1.replace("*NEW* ","");
    } catch(e) {
      txt = t1;
    }
...
googletag.cmd.push(function() { 
  googletag.pubads().addEventListener("slotRenderEnded", function(e) {
    ...
    if(!e.isEmpty && e.size && e.size[0] && e.size[0]>10) {
      ...
      if(div_id.toLowerCase().indexOf('itt'.toLowerCase())>=0) { params='{"backgroundColor":"white"}'; timeout = 10; type_id='103'; params=JSON.parse('{"backgroundColor":"white"}'); }
...
t.size_mapping=[{"device": "all_devices", "adtype": "box", "min_width": 0, "sizes": [[300, 250], [300, 600], [336, 280]]}, {"device": "all_devices", "adtype": "bigbox", "min_width": 0, "sizes": [[300, 250], [300, 600]]}, {"device": "desktop", "adtype": "wide", "min_width": 0, "sizes": [[970, 90], [970, 250], [728, 90]]}, {"device": "mobile", "adtype": "wide", "min_width": 0, "sizes": [[300, 250], [320, 50]]}, {"device": "desktop", "adtype": "zocalo", "min_width": 0, "sizes": [[1, 1], [970, 90], [970, 251], [728, 90]]}, {"device": "mobile", "adtype": "zocalo", "min_width": 0, "sizes": [[1, 1], [320, 50]]}, {"device": "desktop", "adtype": "itt", "min_width": 0, "sizes": [[1, 1], [800, 600], [1000, 540]]}, {"device": "mobile", "adtype": "itt", "min_width": 0, "sizes": [[1, 1], [320, 480]]}];
...
t.run_divs = function(div_id_list) {
  adzone.log("run_divs", (!div_id_list||div_id_list.length<1?"(searching)":div_id_list));
  var prepare=[];
  ...
  googletag.cmd.push(function() {
    ...
    Object.keys(prepare).forEach(function(c, index) {
      ...
      try {
        ...
        adzone.log("**** Revisar defineSlot ", div_id);
        adzone.log(e);
      }
    ...
    if(!t.slot_rendered_added) {
      ...
      googletag.pubads().addEventListener('slotRenderEnded', function(e) {
        if(e.isEmpty && Number.isSafeInteger(parseInt(e.slot.getTargeting("refresh"))) && parseInt(e.slot.getTargeting("refresh"))>0 ) {
          ...
          adzone.log("Will refresh " + e.slot.getSlotElementId() + " in " + parseInt(e.slot.getTargeting("refresh")) + " seconds");
        }
        t.slot_rendered(e);
      });
...
t.run_new = function() {
  ...
  if(t.run_new_count%120==0) {
    adzone.log("Seeking slots, "+(t.run_new_count/120)+" min. since start");
  }
...
if(t.auto_parse_slots=='start') {
  adzone.run_new_interval = window.setInterval(function() {adzone.run_new();},2000);
  window.setTimeout(function(){window.clearInterval(adzone.run_new_interval);},5000);
} else if(t.auto_parse_slots=='auto') {
  window.setInterval(function() {adzone.run_new()},2000);
}
