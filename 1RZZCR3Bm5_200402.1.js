...
googletag.cmd.push(function() { 
  googletag.pubads().addEventListener("slotRenderEnded", function(e) {
    ...
    if(!e.isEmpty && e.size && e.size[0] && e.size[0]>10) {
      ...
      if(div_id.toLowerCase().indexOf('zocalo'.toLowerCase())>=0) { type_id='108'; params=JSON.parse('{}'); }
      if(div_id.toLowerCase().indexOf('itt'.toLowerCase())>=0) { type_id='103'; params=JSON.parse('{}'); }
      if(type_id) {
        var timeout = 10;
