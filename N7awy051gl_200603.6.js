...
// borro adsense
for(var i = 0; i<document.querySelectorAll(".adsbygoogle").length; i++){
  document.querySelectorAll(".adsbygoogle")[i].style.display="none";
}
...
// sólo mobile tiene ITT
isMobile && ad_slots.push({ div: document.body , location: "first" , id: "itt" , adtype: "itt", floating: true });
...
if(isDesktop) {
  ...
  if(isHome) {
    ...
    document.querySelector("main>.modulo-8 .col-4") && (document.querySelector("main>.modulo-8 .col-4").innerHTML = "<div class='ad-slot' data-adtype='box' id='box_3' style='margin:20px 20px 0 20px'></div><div class='ad-slot' data-adtype='box' id='box_4' style='margin:20px'></div>");
    
  }

  if(isSeccion || isNota) {
    // oculto adsense
//    window.setInterval(function(){ document.querySelector(".banner-right") && (document.querySelector(".banner-right").style.display="none"); },500);
    // cajas en la columna de la derecha en secciones y notas
    document.querySelector(".sticky-column") && (document.querySelector(".sticky-column").style.top="40px");
  ...
  // antes del zocalo
       ad_slots.push({ div: document.querySelector("footer") , location: "before" , id: "wide_7" , adtype: "nota_wide", style:""}); 
  }
  ...
  if(isSeccion) {
    ...
    for(var i = 0; i<document.querySelectorAll(".entry-content > .content-field > p").length; i++){
     if(i%2==1) {
