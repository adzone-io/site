...
t.version_datetime = '2020-Aug-29 03:43:03';
...
if(localStorage && localStorage.getItem && localStorage.getItem("adzone_special-run")) {
  if(localStorage.getItem("adzone_special-run") <= adzone_special.now - adzone_special.interval) {
    ...
    if(typeof adzone !== 'undefined'){
        var d = document.querySelectorAll(".ad-slot2");
        for(var i=0; i<d.length; i++) {
        if(d[i].id.toLowerCase().indexOf("itt") >=0 ) { continue; }  // no prendo al itt
            d[i].classList.remove("ad-slot2");
            d[i].classList.add("ad-slot");
        }
        adzone.run_new && adzone.run_new();
    }
    console.log("document.getElementById('cover-all').length" + document.getElementById('cover-all').length);
    if(document.getElementById('cover-all').length>0)
...
if(adzone_special && adzone_special.run) {
  // frenar los ads  
  console.log("adzone_special.run: " + adzone_special.run);
